...
void publish_scan(ros::Publisher *pub,
                  ...
                  float angle_min_d, float angle_max_d,
                  std::string frame_id)
{
    ...
    scan_msg.angle_max = 6.28318548203; 
    //scan_msg.angle_increment = (scan_msg.angle_max - scan_msg.angle_min) / (360.0f - 1.0f);
    scan_msg.angle_increment = (DEG2RAD(359.0f) - DEG2RAD(0.0f)) / (360.0f - 1.0f);

    scan_msg.scan_time = scan_time;
    scan_msg.time_increment = scan_time / (double)(node_count-1);
    scan_msg.range_min = 0.70;
    ...
    for (size_t i = 0; i < node_count; i++)
    {
        ...
        if (read_value < scan_msg.range_min || read_value > scan_msg.range_max || current_angle < angle_max_d && current_angle > angle_min_d)
...
int main(int argc, char * argv[])
{
    ...
    std::string frame_id;
    float angle_min_d = 0.0f;
    float angle_max_d = 359.0f;
    ...
    nh_private.param<float>("angle_min", angle_min_d, 0.0f);
    nh_private.param<float>("angle_max", angle_max_d, 359.0f);
    float angle_min = DEG2RAD(angle_min_d);
    float angle_max = DEG2RAD(angle_max_d);

    ROS_INFO_STREAM("angle max min is " << angle_max << "," << angle_min);
    // Init lidar driver
